...
> Allows to configure the interface -REST or gRPC- and the data format -JSON, XML or YAML- used to communicate
with the verification service, XML and YAML formats exploit an extension of Tosca Specification, see [...]. 
At program start the default configuration uses the gRPC interface with XML data format.
